...
public class MasterBankRequestHandler implements IRequestHandler {
    ...
    public MasterBankRequestHandler(MasterBank bank, NodeOptions nodeOptions, HttpResponseFactory responseFactory) {
        this.bank = bank;
    ...
    private HttpResponse processGetAll() {
        var accounts = bank.getAccounts()
            .stream()
            .map(MasterBankRequestHandler::toResponse)
            .toList();
